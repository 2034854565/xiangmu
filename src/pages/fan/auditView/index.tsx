...
import { fallbackImage, FanListItem, TreeDataItem } from './data';
import {
  getFanVersionData,
  getPassedFanVersionData,
  queryFanData,
  queryFanPerfData,
  queryFansList,
} from './service';
...
const AuditView: React.FC<{ permission: number[]; manage: boolean }> = (props) => {
  ...
  const goFanAlterApplyDetail = (_: any, record: any) => {
    ...
    // console.log(`/fans/create?id=${record.id}&&type=alter`);
    // &&activeKey=${manage ? '5' : '6'}
  ...
  const copyMethodDetail = (_: any, record: any) => {
    // ?id=${record.id}&type=copy&&activeKey=6
    history.push(`/fans/create`, {
      id: record.id,
      type: 'copy',
      activeKey: '6',
  ...
  const filterList = manage
    ...
    : [
        ...
        { text: '变更审核批准中', value: 'auditByLead' },
        { text: '变更审核确认中', value: 'auditAlter' },
      ];
  const columns: ProColumns<FanListItem>[] = [
...

    {
      ...
      render: (_, record) => {
        ...
        // if (record.delete == 1) {
        //   result.push(<Tag color="default">已停用</Tag>);
        //   return result;
        // }
    ...
    return (
      <ProTable
        ...
        request={async (params) => {
          const res = await getPassedFanVersionData({
            id: record.id,
            model: record.model,
